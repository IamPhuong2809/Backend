...
namespace nav2_ipso_planner
{
    ...
    int TrajectoryGeneration::splineOrder = 4; // Set the order of the B-spline curve
    ...
    std::pair<double, double> TrajectoryGeneration::findNearestSafePoint(int x, int y, const unsigned char* global_costmap, int nx, int ny, unsigned char lethal_cost)
    {
        // Kiểm tra xem điểm hiện tại có an toàn không
        if (x >= 0 && x < nx && y >= 0 && y < ny)
        {
            int idx = x + ny * y;
            if (global_costmap[idx] < lethal_cost)
            {
                return {static_cast<double>(x), static_cast<double>(y)};
            }
        }

        //Duyệt các ô lân cận theo bán kính tăng dần (tối đa 5 ô)
        for (int radius = 1; radius <= 5; ++radius)
        {
            for (int dx = -radius; dx <= radius; ++dx)
            {
                for (int dy = -radius; dy <= radius; ++dy)
                {
                    ...
                    {
                        int idx = new_x + ny * new_y;
                        if (global_costmap[idx] < lethal_cost)
                        {
                            return {static_cast<double>(new_x), static_cast<double>(new_y)};
                        }
                    }
                }
            }
        ...
        // Trả lại điểm gốc nếu không tìm thấy (xử lý ở nơi gọi)
        return {static_cast<double>(x), static_cast<double>(y)};
    }
    ...
    const unsigned char* global_costmap, int nx, int ny)
    {
        ...
        // check all points after create B_plan
        for (auto& point : B_plan)
            {
                int x = static_cast<int>(point.first);
                int y = static_cast<int>(point.second);
                auto safe_point = findNearestSafePoint(x, y, global_costmap, nx, ny, nav2_costmap_2d::LETHAL_OBSTACLE);
                // Xử lý điểm không hợp lệ (ví dụ: loại bỏ hoặc điều chỉnh)
                point = safe_point; 
            }
        }
