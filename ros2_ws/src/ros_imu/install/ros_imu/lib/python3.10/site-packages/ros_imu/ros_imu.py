...
def readConfig(device):
    ...
    if (len(tVals)>0):
        print("Return data content, return rate, communication rate:" + str(tVals))
    else:
        print("No return")
    tVals = device.readReg(0x23,2)  #读取安装方向、算法  Read the installation direction and algorithm
    if (len(tVals)>0):
        print("Return the installation direction and algorithm:" + str(tVals))
    else:
        print("No return")
...
def FiledCalibration(device):
    ...
    if input(" Please rotate slowly around the XYZ axis for one circle respectively.\n"
		"After the three axes have completed the rotation, end the calibration（Y/N)？").lower()=="y":
...
def onUpdate(deviceModel):
    ...
    print("Chiptime:" + str(deviceModel.getDeviceData("Chiptime"))
         ...
         ,  "Gyroscope:" + str(deviceModel.getDeviceData("gyroX")) +","+ str(deviceModel.getDeviceData("gyroY")) +","+ str(deviceModel.getDeviceData("gyroZ"))
