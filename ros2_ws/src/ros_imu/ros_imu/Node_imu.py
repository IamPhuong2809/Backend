...
class Node_IMU(Node):
    ...
    def onUpdate(self, deviceModel):
        ...
        yaw = math.radians(deviceModel.getDeviceData("angleX"))
        pitch = math.radians(deviceModel.getDeviceData("angleY"))
        roll = math.radians(deviceModel.getDeviceData("angleZ"))

        q = R.from_euler('xyz', [roll, pitch, yaw]).as_quat()
        imu_msg.orientation = Quaternion(x=q[0], y=q[1], z=q[2], w=q[3])

        self.publisher.publish(imu_msg)
        print(deviceModel.getDeviceData("angleZ"), deviceModel.getDeviceData("angleY"), deviceModel.getDeviceData("angleX"))
